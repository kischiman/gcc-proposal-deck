...
function buildView(force) {
  ...
  return {
    ...
    rates: null,
    // A line hidden from the public does not travel to the public payload at all —
    // not as a row, and not as a figure inside the total. Concealing it in the page
    // would still ship its name and cost to anyone who opened the network tab, and
    // counting it in the total would leave the public sum unreconcilable with the
    // rows on screen. The team and admin views ask with `force` and see everything.
    tasks: (force ? state.tasks : state.tasks.filter((t) => !t.hidden)).map((t) => {
      ...
      return {
        ...
        // Only ever true in a view that asked for everything — the public payload has
        // already dropped these lines, so it can never carry a true here.
        hidden: force ? !!t.hidden : false,
...
/** Keep a line off the public board and the deck, without deleting it. It stays in
 *  full on the team view and in the admin panel, and it keeps counting toward their
 *  totals — only the public payload loses it. */
export function setHidden(id, value) {
  const task = state.tasks.find((t) => t.id === id);
  if (!task) return false;
  task.hidden = Boolean(value);
  save();
  return true;
}
